...
export class CatalogClient {
  ...
  async listProducts(
    ...
    // Supabase's .in() does not accept a subquery — fetch IDs first.
    // If the junction table returns no rows the filter resolves to
    // "id in ()" which is invalid SQL, so we return early with an
    // empty result rather than passing an empty array to .in().
    if (categoryId) {
      ...
      const ids = (rows ?? []).map(
        (r: Record<string, unknown>) => r["product_id"] as string,
      );
      if (ids.length === 0) return buildPaginatedResult([], 0, params);
   ...
   */
  async listCategories(parentId?: string | null): Promise<ProductCategory[]> {
    let query = this.supabase
  ...
  ): Promise<PaginatedResult<ProductCollection>> {
    ...
    return {
      ...
      variants: ((r["product_variants"] as unknown[]) ?? []).map(
        (v) =>
          ({
            ...
            allowBackorder: (v as Record<string, unknown>)[
              "allow_backorder"
            ] as boolean,
            manageInventory: (v as Record<string, unknown>)[
              "manage_inventory"
            ] as boolean,
            rank: (v as Record<string, unknown>)["rank"] as number,
            metadata: (v as Record<string, unknown>)["metadata"] as Record<
              string,
              unknown
            > | null,
            optionValues:
      ...
      options: ((r["product_options"] as unknown[]) ?? []).map((o) => {
        ...
        return {
          ...
          values: ((opt["product_option_values"] as unknown[]) ?? []).map(
            (v) => {
