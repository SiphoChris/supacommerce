...
function installCommand(
  ...
  switch (pm) {
    case "pnpm":
      return `pnpm add${devFlag} ${pkgs}`;
    case "yarn":
      return `yarn add${devFlag} ${pkgs}`;
    case "bun":
      return `bun add${devFlag} ${pkgs}`;
    default:
      return `npm install${devFlag} ${pkgs}`;
  }
}
...
function printHeader(): void {
  console.log();
  console.log(
    chalk.bold.white("  supacommerce") + chalk.dim(" — ecommerce for Supabase"),
  );
  console.log();
}
...
export async function initCommand(options: InitOptions): Promise<void> {
  ...
  if (!fs.existsSync(projectDir)) {
    console.log(
      chalk.dim(`  Directory ${chalk.white(projectDir)} does not exist.`),
    );
  ...
  if (!fs.existsSync(TEMPLATES_DIR)) {
    console.error(
      chalk.red("  Error: templates directory not found at " + TEMPLATES_DIR),
    );
    console.error(
      chalk.dim(
        "  This is a build issue. Please report it at https://github.com/SiphoChris/supacommerce.git",
      ),
    );
    process.exit(1);
  }
  ...
  const mappings: FileMapping[] = [
    ...
    // Import map for edge functions
    {
      ...
      description:
        "Edge function import map (for absolute imports in edge functions)",
    },
    ...
    // config.toml file for overriding the jwt verify done by the supabase platform for functions
    {
      ...
      description:
        "Config file for overriding JWT verification in Supabase Edge Functions",
    },
  ...
  if (missing.length > 0) {
    console.error(
      chalk.red("  Error: the following template files are missing:"),
    );
  ...
  if (willOverwrite.length > 0 && !options.skipConfirmation) {
    console.log(
      chalk.yellow(
        `  ⚠  ${willOverwrite.length} file(s) already exist and will be overwritten.`,
      ),
    );
    console.log();
  }
  ...
  succeeded.forEach((r) => {
    ...
    console.log(
      `  ${label}  ${chalk.cyan(relPath(projectDir, r.mapping.dest))}`,
    );
  });
  ...
  const deps = ["drizzle-orm", "@supabase/supabase-js", "@supacommerce/client"];
  ...
  console.log(
    `     ${chalk.white("mv drizzle.config.example.ts drizzle.config.ts")}`,
  );
  console.log(`     ${chalk.dim("   Add your DATABASE_URL to .env")}`);
  console.log(
    `     ${chalk.dim("   DATABASE_URL=postgresql://postgres:[password]@[host]:5432/postgres")}`,
  );
  console.log();
  ...
  console.log(
    `     ${chalk.white(runCommand(pm, "db:push:remote"))} ${chalk.dim("(or: supabase db push)")}`,
  );
  console.log();

  console.log(chalk.dim("  5. Apply RLS policies and Postgres functions"));
  console.log(
    `     ${chalk.dim("   Paste supabase/rls.sql into your Supabase SQL Editor")}`,
  );
  console.log(
    `     ${chalk.dim("   Paste supabase/functions.sql into your Supabase SQL Editor")}`,
  );
  console.log();
  ...
  console.log(
    chalk.dim('     import { createClient } from "@supacommerce/client"'),
  );
  console.log();
  console.log(
    chalk.dim("     const supabase = createSupabaseClient(url, anonKey)"),
  );
  console.log(chalk.dim("     const commerce = createClient(supabase)"));
  console.log();
  console.log(
    chalk.dim("     const products = await commerce.catalog.listProducts()"),
  );
